...
/* ------------------------------------------------------------------ */
/*  Cabinet Mina BIBAWI — Avocat — Vanilla TypeScript SPA               */
...
function Header(router: Router): HTMLElement {
  ...
  logo.append(
    el('span', '', 'Mina BIBAWI', { style: 'display:block;fontFamily:Georgia,serif;fontSize:1.25rem;fontWeight:700;color:var(--p);lineHeight:1.1' }),
    el('span', '', 'Avocat au Barreau de Paris', { style: 'display:block;fontSize:.7rem;letterSpacing:1px;textTransform:uppercase;color:var(--a);fontWeight:700' })
  );
...
function ExpertiseCard(x: Expertise, router: Router): HTMLElement {
...
function HomePage(router: Router): HTMLElement {
  ...
  const aboutImg = el('img', '', '', { src: `${IMAGES}perso2_retouche.jpg?auto=format&fit=crop&w=900&q=80`, alt: 'Cabinet Mina Bibawi', style: 'borderRadius:var(--ra)' });
...
function AboutPage(): HTMLElement {
  ...
  const img = el('img', '', '', { src: `${IMAGES}perso2_retouche.jpg?auto=format&fit=crop&w=900&q=80`, alt: 'Ma\u00eetre Mina Bibawi', style: 'borderRadius:var(--ra)' });
  w2.append(txt, img);
  s2.appendChild(w2);
  page.appendChild(s2);
...
function ExpertisesPage(router: Router): HTMLElement {
  ...
  const hero = el('section', ['sec', 'hero', 'hero-flat'], '', { style: `background:linear-gradient(rgba(8,23,38,.6),rgba(8,23,38,.6)),url(${IMAGES}hero.png?auto=format&fit=crop&w=1800&q=80) center/cover no-repeat` });
  const heroInner = el('div', 'w');
...
/* Scale of justice */
function iconScale(): SVGSVGElement {
...
/* Building / soci\u00e9t\u00e9s */
function iconBuilding(): SVGSVGElement {
...
/* Briefcase / fonds de commerce */
function iconBriefcase(): SVGSVGElement {
...
function iconContract(): SVGSVGElement {
...
/* Globe / international */
function iconGlobe(): SVGSVGElement {
